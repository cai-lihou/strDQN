...
class TimePosEncode(nn.Module):
    """融合绝对时间编码（TimeEncode）和相对位置编码（PosEncode）的模块"""

    def __init__(self, expand_dim, seq_len, factor=5, fusion_mode='concat'):
        """
        :param expand_dim: 单个编码分支的输出维度
        :param seq_len: 相对位置编码的最大序列长度
        :param factor: TimeEncode的频率缩放因子
        :param fusion_mode: 融合方式：'concat'（拼接，总维度2*expand_dim）或 'add'（相加，总维度expand_dim）
        """
        super().__init__()

        # 初始化两个编码分支
        self.time_encoder = TimeEncode(expand_dim=expand_dim, factor=factor)
        self.pos_encoder = PosEncode(expand_dim=expand_dim, seq_len=seq_len)
        self.projection = nn.Linear(2 * expand_dim, expand_dim)
        # 可选：初始化投影层权重
        torch.nn.init.xavier_normal_(self.projection.weight)
        torch.nn.init.zeros_(self.projection.bias)

        # 融合方式（拼接/相加）
        assert fusion_mode in ['concat', 'add'], "fusion_mode must be 'concat' or 'add'"
        self.fusion_mode = fusion_mode

        # 若选择相加，确保两个分支输出维度一致（已通过expand_dim保证）
        if fusion_mode == 'add':
            # 可选：添加可训练的权重参数平衡两个分支
    ...
    def forward(self, ts):
        """
        :param ts: 输入时间戳张量，形状 [N, L]（N=批次大小，L=序列长度）
        :return: 融合后的编码特征，形状 [N, L, D]（D=2*expand_dim 或 expand_dim）
        """
        # 1. 获取绝对时间编码（捕捉时间周期性、绝对值信息）
        time_emb = self.time_encoder(ts)  # [N, L, expand_dim]

        # 2. 获取相对位置编码（捕捉时间的相对顺序）
        pos_emb = self.pos_encoder(ts)  # [N, L, expand_dim]

        # 3. 融合两种编码
        if self.fusion_mode == 'concat':
            # 拼接：总维度 = expand_dim + expand_dim = 2*expand_dim
            final_emb = torch.cat([time_emb, pos_emb], dim=-1)
            final_emb = self.projection(final_emb)
        else:  # 'add'
            # 相加：总维度 = expand_dim，通过权重平衡
            final_emb = self.time_weight * time_emb + self.pos_weight * pos_emb
...
# ... (前面的 MergeLayer, ScaledDotProductAttention, MultiHeadAttention 等辅助类保持不变) ...

class TGAN(torch.nn.Module):
    def __init__(self, ngh_finder, n_feat, e_feat,
                 attn_mode='prod', use_time='postime', agg_method='attn',
                 node_dim=None, time_dim=None,  # 注意这两个参数
        ...
        # 1. 原始特征加载 (保持不变，这里存的是 172 维的原始数据)
        ...
        self.edge_raw_embed = torch.nn.Embedding.from_pretrained(self.e_feat_th, padding_idx=0,
                                                                 freeze=True)  # 建议 freeze=True
        self.node_raw_embed = torch.nn.Embedding.from_pretrained(self.n_feat_th, padding_idx=0,
                                                                 freeze=True)  # 建议 freeze=True

        # 2. 维度定义
        # raw_feat_dim: 原始特征维度 (172)
        # model_feat_dim: 模型内部维度 (128)，由 node_dim 参数决定

        self.raw_feat_dim = self.n_feat_th.shape[1]  # 172

        if node_dim is not None:
            self.feat_dim = node_dim  # 128
        else:
            self.feat_dim = self.raw_feat_dim  # 兼容旧逻辑

        self.n_feat_dim = self.feat_dim
        self.e_feat_dim = self.feat_dim
        self.model_dim = self.feat_dim

        # 3. [新增] 线性投影层：将 172 维投影到 128 维
        # 注意：这里我们对节点特征和边特征都做投影，确保维度统一
        self.node_feat_proj = torch.nn.Linear(self.raw_feat_dim, self.feat_dim)
        # 如果边特征维度也是 172，也需要投影。通常假设边特征维度与节点特征一致。
        self.edge_feat_proj = torch.nn.Linear(self.e_feat_th.shape[1], self.feat_dim)

        # 初始化投影层权重 (Xavier初始化有助于收敛)
        torch.nn.init.xavier_normal_(self.node_feat_proj.weight)
        torch.nn.init.xavier_normal_(self.edge_feat_proj.weight)
        ...
        # 修改 TimeEncode 的输入维度为投影后的维度 (128)
        if use_time == 'time':
            self.logger.info('Using time encoding')
            self.time_encoder = TimeEncode(expand_dim=self.feat_dim)  # 使用 feat_dim (128)
        elif use_time == 'pos':
        ...
        elif use_time == 'postime':
            self.logger.info('Using postime encoding')
            self.time_encoder = TimePosEncode(expand_dim=self.feat_dim, seq_len=seq_len, factor=5, fusion_mode='concat')
        else:
    ...
    def tem_conv(self, src_idx_l, cut_time_l, curr_layers, num_neighbors=20):
        ...
        # [修改] 时间编码使用投影后的维度 (128)
        # 这里传入 zeros，TimeEncoder 会返回相应维度的向量
        src_node_t_embed = self.time_encoder(torch.zeros_like(cut_time_l_th))

        # [核心修改] 获取原始特征并投影
        src_node_raw = self.node_raw_embed(src_node_batch_th)  # [B, 172]
        src_node_feat = self.node_feat_proj(src_node_raw)  # [B, 128]
        ...
        if curr_layers == 0:
            ...
            # [核心修改] 获取原始边特征并投影
            src_ngn_edge_raw = self.edge_raw_embed(src_ngh_eidx_batch)  # [B, N, 172]
            src_ngn_edge_feat = self.edge_feat_proj(src_ngn_edge_raw)  # [B, N, 128]
